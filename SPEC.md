I want an [[Obsidian]] plugin that will make entire links (wikilinks and Markdown links) act like single words when I use the "option" (Mac) or Ctrl (Windows) keys to make the arrow keys operate in word-sized increments. When selecting, it would select the entire link with one keystroke. When deleting, it would delete the entire link with one press of Backspace or Delete, as if the link were one single word. Make sure that it supports both wikilinks (with aliases, and with folder paths and separators) and Markdown links.

Obey the global (app/user/OS) setting for which key causes the arrow keys to jump a word at a time (whether that's Option, or something else).
...
- If the cursor is inside a link, then act in the normal way: don't override the behavior of the arrow keys. The user probably wants to jump normally (i.e., not across the entire link).